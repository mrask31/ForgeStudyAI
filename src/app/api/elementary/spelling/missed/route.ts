...
export async function GET(req: Request) {
  try {
    ...
    let query = supabase
      ...
      .select('word_id, spelling_words(word)')
    ...
    if (error) {
      console.error('[Spelling Missed] Error:', error)
      return NextResponse.json({ error: 'Failed to load missed words' }, { status: 500 })
    }

    const words = (data || [])
      .map((row: any) => row.spelling_words?.word)
      .filter(Boolean)
